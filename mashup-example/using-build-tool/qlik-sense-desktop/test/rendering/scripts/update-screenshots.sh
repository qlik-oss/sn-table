...
# Create and start the container with the name *sn-table-mashup-playwright*
...
docker run -d --name sn-table-mashup-playwright --rm --network host -v $(pwd):/work/ -w /work/ -it mcr.microsoft.com/playwright:v$playwrightVersion-focal

# The actual generate/update the reference screenshots is running from the machine running this shell script
docker exec sn-table-mashup-playwright /bin/sh -c "yarn && yarn test:rendering --update-snapshots"

docker stop sn-table-mashup-playwright
