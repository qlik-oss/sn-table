...
export const getSelectionMouseHandlers = (
  ...
  const handleMouseDown = (evt: React.MouseEvent) => {
    ...
    if (areBasicFeaturesEnabled && cell.isSelectable) {
      const mouseupCallback = () => selectionDispatch({ type: SelectionActions.SELECT_MULTI_END });
      selectionDispatch({ type: SelectionActions.SELECT_MOUSE_DOWN, payload: { cell, mouseupCallback } });
    }
  };
